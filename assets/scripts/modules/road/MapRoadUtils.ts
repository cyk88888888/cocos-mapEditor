...
export default class MapRoadUtils {
	...
	public updateMapInfo(mapWidth: number, mapHeight: number, nodeWidth: number, nodeHeight: number, mapType: CONST.MapType): void {
		...
		switch (this._mapType) {
			...
			case CONST.MapType.honeycomb2:
				...
				this._mapRoad = new MapRoadHoneycomb2(this._row, this._col, this._nodeWidth, this._nodeHeight, this._halfNodeWidth, this._halfNodeHeight);
	...
	*/
	/*	public getWorldPointByDerect(dx:number,dy:number):Point
		{
			let col:number = (dy + dx) / 2;
			let row:number = (dy - dx) / 2 + col - 1;
...
class MapRoad45Angle implements IMapRoad {
	 ...
	 */
	public getNodeByPixel(x: number, y: number): RoadNode {
		let wPoint: Point = this.getWorldPointByPixel(x, y);
		let fPoint: Point = this.getPixelByWorldPoint(wPoint.x, wPoint.y);
		let dPoint: Point = this.getDerectByPixel(x, y);

		let node: RoadNode = new RoadNode();
		...
		node.dx = node.col = dPoint.x;
		node.dy = node.row = dPoint.y;
	...
	public getNodeByDerect(dx: number, dy: number): RoadNode {
		...
		let fPoint: Point = this.getPixelByDerect(dx, dy);
		let wPoint: Point = this.getWorldPointByPixel(fPoint.x, fPoint.y);
	 ...
	 */
	public getNodeByWorldPoint(cx: number, cy: number): RoadNode {
		let point: Point = this.getPixelByWorldPoint(cx, cy)
	 ...
	 */
	public getWorldPointByPixel(x: number, y: number): Point {
		let cx: number = Math.ceil(x / this._nodeWidth - 0.5 + y / this._nodeHeight) - 1;
		let cy: number = (this._col - 1) - Math.ceil(x / this._nodeWidth - 0.5 - y / this._nodeHeight);
	 ...
	 */
	public getPixelByWorldPoint(cx: number, cy: number): Point {
		let x: number = Math.floor((cx + 1 - (cy - (this._col - 1))) * this._halfNodeWidth);
		let y: number = Math.floor((cx + 1 + (cy - (this._col - 1))) * this._halfNodeHeight);
	 ...
	 */
	public getDerectByPixel(x: number, y: number): Point {
		let worldPoint: Point = this.getWorldPointByPixel(x, y);
		let pixelPoint: Point = this.getPixelByWorldPoint(worldPoint.x, worldPoint.y);
		let dx: number = Math.floor(pixelPoint.x / this._nodeWidth) - (pixelPoint.x % this._nodeWidth == 0 ? 1 : 0);
		let dy: number = Math.floor(pixelPoint.y / this._halfNodeHeight) - 1;
	 ...
	 */
	public getDerectByWorldPoint(cx: number, cy: number): Point {
		let dx: number = Math.floor((cx - (cy - (this._col - 1))) / 2);
		let dy: number = cx + (cy - (this._col - 1));
		return new Point(dx, dy);
	}
	 ...
	 */
	public getPixelByDerect(dx: number, dy: number): Point {
		let x: number = Math.floor((dx + dy % 2) * this._nodeWidth + (1 - dy % 2) * this._halfNodeWidth);
		let y: number = Math.floor((dy + 1) * this._halfNodeHeight);
		return new Point(x, y);
	}
}
...
class MapRoad90Angle implements IMapRoad {
	 ...
	 */
	public getWorldPointByPixel(x: number, y: number): Point {
		let cx: number = Math.floor(x / this._nodeWidth);
		let cy: number = Math.floor(y / this._nodeHeight);
	 ...
	 */
	public getPixelByWorldPoint(cx: number, cy: number): Point {
		let x: number = Math.floor((cx + 1) * this._nodeWidth - this._halfNodeWidth);
		let y: number = Math.floor((cy + 1) * this._nodeHeight - this._halfNodeHeight);
		return new Point(x, y);
	}
	 ...
	 */
	public getDerectByPixel(x: number, y: number): Point {
		let dx: number = Math.floor(x / this._nodeWidth);
		let dy: number = Math.floor(y / this._nodeHeight);
		return new Point(dx, dy);
	}
	 ...
	 */
	public getPixelByDerect(dx: number, dy: number): Point {
		let x: number = Math.floor((dx + 1) * this._nodeWidth - this._halfNodeWidth);
		let y: number = Math.floor((dy + 1) * this._nodeHeight - this._halfNodeHeight);
		return new Point(x, y);
	}
}
...
class MapRoadHoneycomb implements IMapRoad {
	...
	public getNodeByPixel(x: number, y: number): RoadNode {
		let wPoint: Point = this.getWorldPointByPixel(x, y);
		let fPoint: Point = this.getPixelByWorldPoint(wPoint.x, wPoint.y);
		//let dPoint:Point = getDerectByPixel(x,y);
		...
		node.dx = node.col = wPoint.x;
		node.dy = node.row = wPoint.y;
	...
	public getNodeByDerect(dx: number, dy: number): RoadNode {
		...
		let fPoint: Point = this.getPixelByDerect(dx, dy);
		//let wPoint:Point = getWorldPointByPixel(fPoint.x,fPoint.y);
	 ...
	 */		
	 public getWorldPointByPixel(x:number,y:number):Point
	 {
		let nwDiv4Index:number = Math.floor(x / this._nwDiv4);    //六边形的外切矩形竖方向均等分成4分，所有的六边形外切矩形连在一起形成一个个4分之一矩形宽的区域，nwDiv4Index就是该区域的索引
		 
		let col:number = Math.floor(nwDiv4Index / 3);    //取得临时六边形横轴的索引,根据不同的情况可能会变
	...
	private testPointInHoneycomb(col: number, row: number, px: number, py: number): Boolean {
		...
		let a: number = this._nwDiv4 * 2;

		let point: Point = this.getPixelByWorldPoint(col, row);

		let absX: number = Math.abs(px - point.x);
		let absY: number = Math.abs(py - point.y);
	 ...
	 */
	public getPixelByWorldPoint(cx: number, cy: number): Point {
		let x: number = Math.floor((2 + 3 * cx) / 4 * this._nodeWidth);
		let y: number = Math.floor((cy + 1 / 2 * (1 + (cx % 2))) * this._nodeHeight);
	 ...
	 */
	public getPixelByDerect(dx: number, dy: number): Point {
		let x: number = (2 + 3 * dx) / 4 * this._nodeWidth;
		let y: number = (dy + 1 / 2 * (1 + (dx % 2))) * this._nodeHeight;
		return new Point(x, y);
	}
}
...
class MapRoadHoneycomb2 implements IMapRoad {
	...
	private transposedNode(node: RoadNode): RoadNode {
		...
		node.dx = node.col = node.dy;
		node.dy = node.row = temp_dx;
	 ...
	 */
	public getNodeByWorldPoint(cx: number, cy: number): RoadNode {
		return this.transposedNode(this.mapRoadHoneycomb.getNodeByWorldPoint(cx, cy));
	}
