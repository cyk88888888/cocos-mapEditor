...
@ccclass('Player11')
export class Player11 extends UIComp {
    @property({ type: Node })
    ...
    public moveByJoyStick(radian: number) {
        ...
        let cellWidth = mapMgr.cellWidth;
        let cellHeight = mapMgr.cellHeight;
        let dir = cos > 0 ? 1 : -1;
        let isCanMoveX = true;
        ...
        let startRow = Math.round(startY / cellHeight);
        let endRow = Math.round(endY / cellHeight);
        let circleData = { cx: toX + self._cx, cy: playerPos.y + self._cy, r: self._cRadius };
        for (let i = startRow, len = endRow; i < len; i++) {
            let rectData = { x: col * cellWidth, y: i * cellHeight, w: cellWidth, h: cellHeight };
        ...
        let startCol = Math.round(startX / cellWidth);
        let endCol = Math.round(endX / cellWidth);
        for (let j = startCol, len = endCol; j < len; j++) {
            let rectData = { x: j * cellWidth, y: row * cellHeight, w: cellWidth, h: cellHeight };
            if (!BaseUT.RectCircleColliding(circleData, rectData)) continue;
